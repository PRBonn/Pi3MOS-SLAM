...
std::vector<torch::Tensor> cuda_ba(
    ...
    torch::Tensor kk,
    const int PPF,
    int t0, int t1, int iterations, bool eff_impl, bool use_cov_adaptive,
    float cov_steepness, float cov_threshold);
...
std::vector<torch::Tensor> ba(
    ...
    int t0, int t1, int iterations, bool eff_impl, bool use_cov_adaptive,
    float cov_steepness, float cov_threshold) {
  return cuda_ba(poses, patches, intrinsics, target, weight, prior_ds, prior_ws,
                 lmbda, ii, jj, kk, PPF, t0, t1, iterations, eff_impl, use_cov_adaptive,
                 cov_steepness, cov_threshold);
}
