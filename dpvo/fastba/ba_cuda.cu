...
// per-patch fused kernel for depth prior: reads center inv-depth and accumulates prior
__global__ void add_depth_prior_fused(
    ...
    const torch::PackedTensorAccessor32<float,1,torch::RestrictPtrTraits> prior_ds,
    const torch::PackedTensorAccessor32<float,1,torch::RestrictPtrTraits> prior_ws,
    const torch::PackedTensorAccessor32<mtype,1,torch::RestrictPtrTraits> uncertainty_ws,
    torch::PackedTensorAccessor32<mtype,1,torch::RestrictPtrTraits> C,
    torch::PackedTensorAccessor32<mtype,1,torch::RestrictPtrTraits> u) 
{
  GPU_1D_KERNEL_LOOP(m, kx.size(0)) {
    const long k = kx[m];
    const float z = patches[k][2][1][1];
    const float wp = fmaxf(0.f, prior_ws[k]);
    if (wp == 0.f) continue;
    const float s = (float)uncertainty_ws[m];
    const float wp_eff = wp * s;
    if (wp_eff == 0.f) continue;
    // const float wp_eff = wp;
    C[m] += wp_eff;
    u[m] += wp_eff * (prior_ds[k]-z);
  }
}
    ...
    torch::PackedTensorAccessor32<mtype,1,torch::RestrictPtrTraits> u, const int t0, const int ppf)
{
  ...
  GPU_1D_KERNEL_LOOP(n, ii.size(0)) {
    ...
    // fetch per-frame intrinsics
    const float fxi = intrinsics[ix][0];
    const float fyi = intrinsics[ix][1];
    const float cxi = intrinsics[ix][2];
    const float cyi = intrinsics[ix][3];
    const float fxj = intrinsics[jx][0];
    const float fyj = intrinsics[jx][1];
    const float cxj = intrinsics[jx][2];
    const float cyj = intrinsics[jx][3];

    float Xi[4], Xj[4];
    Xi[0] = (patches[kx][0][1][1] - cxi) / fxi;
    Xi[1] = (patches[kx][1][1][1] - cyi) / fyi;
    Xi[2] = 1.0;
    ...
    for (int row=0; row<2; row++) {
      ...
      if (row == 0){
        ...
        Jz = fxj * (tij[0] * d - tij[2] * (X * d2));
        Jj = (float[6]){fxj*W*d, 0, fxj*-X*W*d2, fxj*-X*Y*d2, fxj*(1+X*X*d2), fxj*-Y*d};
        ...
        Jz = fyj * (tij[1] * d - tij[2] * (Y * d2));
        Jj = (float[6]){0, fyj*W*d, fyj*-Y*W*d2, fyj*(-1-Y*Y*d2), fyj*(X*Y*d2), fyj*X*d};
...
__global__ void reproject(
  ...
  GPU_1D_KERNEL_LOOP(n, ii.size(0)) {
    ...
    // per-edge intrinsics: source (ix) for backprojection, target (jx) for projection
    const float fxi = intrinsics[ix][0];
    const float fyi = intrinsics[ix][1];
    const float cxi = intrinsics[ix][2];
    const float cyi = intrinsics[ix][3];
    ...
    for (int i=0; i<patches.size(2); i++) {
      for (int j=0; j<patches.size(3); j++) {
        ...
        // per-edge intrinsics
        Xi[0] = (patches[kx][0][i][j] - cxi) / fxi;
        Xi[1] = (patches[kx][1][i][j] - cyi) / fyi;
        Xi[2] = 1.0;
        Xi[3] = patches[kx][2][i][j];

        actSE3(tij, qij, Xi, Xj);

        coords[n][0][i][j] = fxj * (Xj[0] / Xj[2]) + cxj;
        coords[n][1][i][j] = fyj * (Xj[1] / Xj[2]) + cyj;
...
std::vector<torch::Tensor> cuda_ba(
    ...
    torch::Tensor target,
    torch::Tensor weight,
    torch::Tensor prior_ds,
    torch::Tensor prior_ws,
    torch::Tensor lmbda,
    torch::Tensor ii,
    torch::Tensor jj,
    torch::Tensor kk,
    const int PPF,
    const int t0, const int t1, const int iterations, bool eff_impl, bool use_cov_adaptive,
    const float cov_steepness, const float cov_threshold)
{
  ...
  // Declare frame_avg_std at function scope
  torch::Tensor depth_std;
  torch::Tensor uncertainty_ws;
  ...
  for (int itr=0; itr < iterations; itr++) {
    ...
    if (itr == 0) {
      torch::Tensor C_inv = 1.0 / (C + lmbda);
      torch::Tensor C_schur_complement;
      torch::Tensor E_Cinv;
      
      if (eff_impl) {
          torch::Tensor EQEt = blockE->computeEQEt(N, C_inv.view({1, M}));
          C_schur_complement = B - EQEt;
          E_Cinv = E * C_inv.view({1, M});
      } else {
          E_Cinv = E * C_inv.view({1, M});
          torch::Tensor Et = torch::transpose(E, 0, 1);
          C_schur_complement = B - torch::matmul(E_Cinv, Et);
      }
   
      torch::Tensor I = torch::eye(6*N, mdtype);
      C_schur_complement += I * (1e-4 * C_schur_complement + 1.0);
      torch::Tensor C_schur_L = std::get<0>(at::linalg_cholesky_ex(C_schur_complement));
  
      auto triangular_result = torch::triangular_solve(I, C_schur_L, /*upper=*/false, /*transpose=*/false);
      torch::Tensor C_schur_L_inv = std::get<0>(triangular_result);
      torch::Tensor F = torch::matmul(C_schur_L_inv, E_Cinv);
  
      torch::Tensor Ft = torch::transpose(F, 0, 1);
      torch::Tensor FtF = torch::matmul(Ft, F);
      torch::Tensor depth_cov = C_inv + torch::diag(FtF);
  
      depth_std = torch::sqrt(depth_cov);
    
      torch::Tensor inv_depths = patches.index({kx, 2, 1, 1});  // extract inverse depths
      depth_std = depth_std / (inv_depths + 1e-6);  // avoid division by zero
      
      if (use_cov_adaptive) {
          depth_std = depth_std.view({-1, PPF});
          torch::Tensor frame_avg_std = std::get<0>(depth_std.median(1));
          
          torch::Tensor frame_avg_std_mapped = 1.0 / (1.0 + torch::exp(-cov_steepness * (frame_avg_std - cov_threshold)));
          
          uncertainty_ws = frame_avg_std_mapped.repeat_interleave(depth_std.size(1));
      } else {
          uncertainty_ws = torch::ones({M}, mdtype);
          torch::Tensor frame_avg_std = torch::ones({1}, mdtype);
      }
    }

    add_depth_prior_fused<<<NUM_BLOCKS(M), NUM_THREADS>>>(
        kx.packed_accessor32<long,1,torch::RestrictPtrTraits>(),
        patches.packed_accessor32<float,4,torch::RestrictPtrTraits>(),
        prior_ds.packed_accessor32<float,1,torch::RestrictPtrTraits>(),
        prior_ws.packed_accessor32<float,1,torch::RestrictPtrTraits>(),
        uncertainty_ws.packed_accessor32<mtype,1,torch::RestrictPtrTraits>(),
        C.packed_accessor32<mtype,1,torch::RestrictPtrTraits>(),
        u.packed_accessor32<mtype,1,torch::RestrictPtrTraits>());
    ...
    if (t1 - t0 == 0) {
      ...
      if (eff_impl) {
        ...
        // keep Schur complement from the latest iteration (already stored undamped)
      }
